...
if getuser() == "root" and not configuration.root_execute:
    print("{0}error:{1} this program is not allowed to be used as the root user"\
            .format(configuration.color_error, configuration.color_normal))
...
# deletes the downloaded tar to prevent duplicates and confusion.
    ...
    print ("{0}extracting downloaded tarball."\
            .format(configuration.color_progress))
    tar.extractall()
    tar.close()
    print ("{0}extracted:{1} downloaded {2}.tar.gz has been extracted"\
            .format(configuration.color_successful, configuration.color_normal, package_name))
    try:
        subprocess.Popen("rm -rf {0}".format(tar_package), shell=True)
        wait()
        print ("{0}removed: {1}tar package has been removed \
                and the contents has been stored in {2}/{3}"\
                .format(configuration.color_progress, configuration.color_normal, directory, package_name))
    except:
        print ("{0}error:{1} can't remove downloaded tarball,\
                please remove it manually").format(configuration.color_error, configuration.color_normal)
...
if args.S: 
    ...
    print("{0}error:{1} no argument given. launch 'pacwoman -h' to know all the options available"\
            .format(configuration.color_error, configuration.color_normal))
